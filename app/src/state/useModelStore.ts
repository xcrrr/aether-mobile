...
interface ModelState {
  ...
  reattachDownloads: () => Promise<void>;
...
type ModelManagerModule = typeof import('@/models/ModelManager');

function getModelManager(): ModelManagerModule {
  return require('@/models/ModelManager') as ModelManagerModule;
}
...
export const useModelStore = create<ModelState>((set, get) => ({
  ...
  remove: async (id) => {
    ...
    if (get().activeModelId === id) {
      const installed = get().installed;
      const next = installed[DEFAULT_MODEL_ID]
        ? DEFAULT_MODEL_ID
        : MODELS.find((m) => installed[m.id])?.id ?? null;
      await get().setActive(next);
    }
  },
  resetLocalState: () => set({ installed: {}, downloads: {}, activeModelId: null }),
}));
