...
export function AttachmentSheet({ visible, onClose, onCamera, onLibrary, onFiles, onPaste }: {
  ...
  const rows: Row[] = [
    { key: 'camera', icon: <Camera size={20} color={c.textMuted} />, label: 'Camera', sub: 'Take a photo to ask about', onPress: pick(onCamera) },
    { key: 'library', icon: <ImageIcon size={20} color={c.textMuted} />, label: 'Photo Library', sub: 'Choose an existing image', onPress: pick(onLibrary) },
    { key: 'files', icon: <FileText size={20} color={c.textMuted} />, label: 'Files', sub: 'PDF, Word, text', onPress: pick(onFiles) },
  ];
  if (hasImage) {
    rows.push({ key: 'paste', icon: <ClipboardPaste size={20} color={c.textMuted} />, label: 'Paste image', sub: 'Use the image on your clipboard', onPress: pick(onPaste) });
  }
...
const makeStyles = (c: Palette) => StyleSheet.create({
  ...
  iconWrap: { width: 40, height: 40, borderRadius: radius.sm, backgroundColor: c.assistantBubble, alignItems: 'center', justifyContent: 'center' },
  rowLabel: { fontFamily: fonts.sansMedium, fontSize: fontSize.body, color: c.text },
  rowSub: { fontFamily: fonts.sans, fontSize: fontSize.sm, color: c.textMuted, marginTop: 1 },
});
