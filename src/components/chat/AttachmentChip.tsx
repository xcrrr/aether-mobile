  ...
  if (error) {
    return (
      <Pressable style={[styles.chip, styles.errorChip]} onPress={() => onPressError(error)}>
        ...
        <Text style={[styles.name, { color: colors.danger }]} numberOfLines={1}>
          Attachment failed
        </Text>
...
const styles = StyleSheet.create({
  ...
  errorChip: { borderColor: colors.danger, backgroundColor: colors.dangerBg },
  warnChip: { borderColor: '#5C4A12' },
  thumb: { width: 40, height: 40, borderRadius: radius.sm, backgroundColor: colors.assistantBubble },
  fileIcon: { width: 40, height: 40, borderRadius: radius.sm, backgroundColor: colors.assistantBubble, alignItems: 'center', justifyContent: 'center' },
  name: { fontFamily: fonts.sansMedium, fontSize: 13, color: colors.text },
  meta: { fontFamily: fonts.sans, fontSize: 11, color: colors.textMuted, marginTop: 1 },
  ...
  badgeText: { fontFamily: fonts.sans, fontSize: 11, color: '#EAB308', flexShrink: 1 },
});
